...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "SELECT DISTINCT p FROM Product p " +
            ...
            "left join fetch p.images i " +
            // 상태가 '판매'로 되어있는 것을 찾아냄
            "where m.id = :memberId AND p.status = 'SOLD'",
            countQuery = "SELECT count(p) FROM Product p where p.seller.id = :memberId AND p.status = 'SOLD'")
    // 여기서 @Param은 JPQL 쿼리문안에서 : 로 시작하는 부분에 붙여줌
    // 이름 기반 파라미터라고 부름
    // Spring Data JPA는 메서드가 호출될 때 전달된 memberId라는 값을 찾아서 쿼리문에 붙여줌
    Page<Product> findProductByMemberIdWithDetailsAndSOLD(@Param("memberId") Long memberId, Pageable pageable);
